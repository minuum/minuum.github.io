...
제가 선행 VLA 연구에서 구현한 PaliGemma2 기반 visual grounder와 Kosmos-2 비전 인코더 결합 파이프라인 경험, 그리고 시각 정보 오탐지를 극복하기 위해 수행했던 masking ablation 및 zero-shot visual grounding 설계 노하우는 LAIR Lab의 실시간 3D 장면 표현을 VLA의 Context Vector와 결합하여 고차원 인지 능력을 로봇 행동으로 연결하는 연구에 유용한 시너지가 될 것입니다.
